 ...
 * This kernel is invoked by PyTorchForce to calculate the forces acting on the system and the energy of the system.
 */
class CalcPyTorchForceE2EDirectKernel : public OpenMM::KernelImpl {
public:
	static std::string Name() {
	return "CalcPyTorchForceE2EDirect";
	}
	CalcPyTorchForceE2EDirectKernel(std::string name, const OpenMM::Platform& platform) : OpenMM::KernelImpl(name, platform) {
	}
	/**
	 * Initialize the kernel.
	 *
	 ...
	 * @param module         the Pytorch model to use for computing forces and energy
	 */
  virtual void initialize(const OpenMM::System& system, const PyTorchForceE2EDirect& force,
						  torch::jit::script::Module& nnModule) = 0;
						  // std::vector<std::vector<double>> initialSignals,
						  // std::vector<std::vector<double>> targetFeatures) = 0;
  //at::ScalarType positionsType, at::ScalarType boxType, at::ScalarType energyType,
 ...
 * This kernel is invoked by PyTorchForce to calculate the forces acting on the system and the energy of the system.
 */
class CalcPyTorchForceE2EDiffConfKernel : public OpenMM::KernelImpl {
	public:
		static std::string Name() {
		return "CalcPyTorchForceE2EDiffConf";
		}
		CalcPyTorchForceE2EDiffConfKernel(std::string name, const OpenMM::Platform& platform) : OpenMM::KernelImpl(name, platform) {
		}
		/**
		 * Initialize the kernel.
		 *
		 ...
		 * @param module         the Pytorch model to use for computing forces and energy
		 */
	  virtual void initialize(const OpenMM::System& system, const PyTorchForceE2EDiffConf& force,
							  torch::jit::script::Module& nnModule) = 0;
							  // std::vector<std::vector<double>> initialSignals,
							  // std::vector<std::vector<double>> targetFeatures) = 0;
	  //at::ScalarType positionsType, at::ScalarType boxType, at::ScalarType energyType,

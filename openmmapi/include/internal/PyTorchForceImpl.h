...
/**
 * This is the internal implementation of PyTorchForceE2EDiffConf.
 */

 class OPENMM_EXPORT_PYTORCH PyTorchForceE2EDiffConfImpl : public OpenMM::ForceImpl {
	public:
		PyTorchForceE2EDiffConfImpl(const PyTorchForceE2EDiffConf& owner);
		~PyTorchForceE2EDiffConfImpl();
		void initialize(OpenMM::ContextImpl& context);
		const PyTorchForceE2EDiffConf& getOwner() const {
		return owner;
		}
		void updateContextState(OpenMM::ContextImpl& context, bool& forcesInvalid) {
		// This force field doesn't update the state directly.
		...
		std::vector<std::string> getKernelNames();
	private:
		const PyTorchForceE2EDiffConf& owner;
		OpenMM::Kernel kernel;
		torch::jit::script::Module nnModule;
		std::vector<int> particleIndicies;
	};
